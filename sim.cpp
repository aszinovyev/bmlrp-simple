...
vector<float> GetAverageNodeDegrees(const Network& net_level0, int max_level) {
  myassert(max_level <= static_cast<int>(sizeof(Addr) * 8));
  myassert(IsGraphConnected(net_level0.graph));

  Graph graph = net_level0.graph;

  vector<float> res;
  res.push_back(static_cast<float>(graph.edges.size()) / graph.n);
...
void ScalePoints(vector<Point>* points) {
  ...
  for (uint i = 0; i < points->size(); ++i) {
    points->at(i).x = (points->at(i).x - minx) / (maxx - minx);
    points->at(i).y = (points->at(i).y - miny) / (maxy - miny);
  }
}
...
Network Manual0() {
  ...
  vector<Addr> addrs = {GenAddr(0), GenAddr(0), GenAddr(1), GenAddr(0),
                        GenAddr(1)};

  vector<Point> points = {{1, 1}, {2, 1}, {1, 0}, {0, 0}, {0, 1}};
  ScalePoints(&points);
...
Network Manual1() {
  ...
  vector<Point> points = {{0, 1}, {1, 1}, {2, 1}, {3, 1}, {4, 1}, {2, 0}};
  ScalePoints(&points);
...
Network Manual2() {
  ...
  vector<Point> points = {{0, 3}, {1, 3}, {2, 3}, {3, 3},
                          {4, 3}, {5, 3}, {6, 3}, {7, 3},
                          {8, 3}, {4, 2}, {5.33, 2}, {4, 1},
                          {4.66, 1}, {4, 0}};
  ScalePoints(&points);
...
Network Manual3() {
  ...
  vector<Point> points = {{0, 2}, {0.5, 1}, {1.5, 1}, {0, 0},
                          {1, 0}, {2, 0}, {3, 0}, {4, -1},
                          {5, -1}};
  ScalePoints(&points);
...
Network Manual4() {
  ...
  vector<Addr> addrs = {
      GenAddr(1), GenAddr(0), GenAddr(0), GenAddr(1),
      GenAddr(0), GenAddr(0), GenAddr(0), GenAddr(0),
      GenAddr(1), GenAddr(1)};
...
Network Manual5() {
  ...
  vector<Addr> addrs = {
      GenAddr(28, 5), GenAddr(12, 5), GenAddr(18, 5), GenAddr(25, 5),
  ...
  cout << AddrToString(GenAddr(28, 5)) << endl;

  vector<Point> points = {{0, 0}, {1, 0}, {2, 0}, {3, 0},
